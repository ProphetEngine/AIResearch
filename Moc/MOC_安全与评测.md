---
title: MOC · 安全与评测
date: 2026-09-23
type: moc
cluster: F
status: active
---

# MOC · 安全与评测

红队与对抗评测、部署防御（分类器 / 熔断）、提示注入架构、可扩展监督与 Safety Cases、评测污染与排行榜可靠性；机制可解释与治理交界亦挂本簇。

## 时间线入口

- [[安全治理评测发展时间线|安全 · 治理 · 评测可靠性时间线]]

## 通史 / 总览节点

- [[安全红队与对抗评测|B5 红队与对抗评测]]
- [[评测与排行榜可靠性]]
- [[模型卡与SystemCard规范|B11 Model Card / System Card 规范]]（与 J 双挂）

## 增量节点

- [[可扩展监督与弱到强|Scalable Oversight / W2S]]
- [[训练数据污染检测|数据污染检测]]
- [[机制可解释性入门|机制可解释性]]
- [[宪法分类器防御|Constitutional Classifiers]]
- [[隐私与机器遗忘|Privacy / Unlearning]]
- [[安全论证SafetyCases|Safety Cases]]
- [[SHADEArena隐瞒与监控|SHADE-Arena]]
- [[AIControl协议与Scheming倾向|AI Control / Scheming Propensity]]
- [[网络防御基准|Cyber Defense Benchmark]]
- [[恶意软件分析评测|Malware Analysis Evals]]
- [[LLM水印|LLM Watermarking]]
- [[审慎对齐与断路器|Deliberative / Circuit Breakers]]
- [[Inspect评测Harness|Inspect Eval Harness]]
- [[激活操控与表征工程|Activation Steering]]
- [[多模态越狱与OmniSafe|MMJail / OmniSafe-MM]]
- [[评测污染可靠性鸿沟|污染可靠性 / 抗污染基准]]
- [[CircuitLasso与电路追踪|CircuitLasso / Circuit Tracing]]
- [[Cascade与SAUL遗忘方法|Cascade · SAUL]]
- [[Prompt注入架构防御|提示注入架构：CaMeL / StruQ]]
- [[StatutoryAI法律规范对齐|Statutory AI]]（与 E 双挂）

## 相关主题

- [[MOC_对齐与强化学习|E 对齐 · RL]]
- [[MOC_模型与技术报告|J 模型谱系与 TR]]
- [[MOC_Harness|D 智能体]]（智能体场景安全）

